# Trains ARS (SB3-Contrib) on a Gymnasium environment, logs episode returns/lengths,
...
def main():
    ...
    # -------------------------
    parser = argparse.ArgumentParser(description="Simple ARS training script (full episode logging)")
    ...
    # -------------------------
    out_dir = os.path.join("output", "ARS", args.env_name)
    ...
    if len(episode_returns) > 0:
        ...
        plt.title(f"{args.env_name} – ARS Training")
    ...
    csv_path = os.path.join("output", "ARS", "final_performance.csv")
