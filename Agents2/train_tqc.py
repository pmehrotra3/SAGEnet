# Trains TQC (SB3-Contrib) on a Gymnasium environment, logs episode returns/lengths,
...
def main():
    ...
    # -------------------------
    parser = argparse.ArgumentParser(description="Simple TQC training script (full episode logging)")
    ...
    # -------------------------
    out_dir = os.path.join("output", "TQC", args.env_name)
    ...
    if len(episode_returns) > 0:
        ...
        plt.title(f"{args.env_name} – TQC Training")
    ...
    csv_path = os.path.join("output", "TQC", "final_performance.csv")
