# Trains TRPO (SB3-Contrib) on a Gymnasium environment, logs episode returns/lengths,
...
def main():
    ...
    # -------------------------
    parser = argparse.ArgumentParser(description="Simple TRPO training script (full episode logging)")
    ...
    # -------------------------
    out_dir = os.path.join("output", "TRPO", args.env_name)
    ...
    if len(episode_returns) > 0:
        ...
        plt.title(f"{args.env_name} – TRPO Training")
    ...
    csv_path = os.path.join("output", "TRPO", "final_performance.csv")
