# Trains SAC on a Gymnasium environment, logs episode returns/lengths,
...
def main():
    ...
    # -------------------------
    parser = argparse.ArgumentParser(description="Simple SAC training script (full episode logging)")
    ...
    # -------------------------
    out_dir = os.path.join("output", "SAC", args.env_name)
    ...
    if len(episode_returns) > 0:
        ...
        plt.title(f"{args.env_name} – SAC Training")
    ...
    csv_path = os.path.join("output", "SAC", "final_performance.csv")
