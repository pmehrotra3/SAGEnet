# Trains PPO on a Gymnasium environment, logs episode returns/lengths,
...
def main():
    ...
    # -------------------------
    parser = argparse.ArgumentParser(description="Simple PPO training script (full episode logging)")
    ...
    # -------------------------
    out_dir = os.path.join("output", "PPO", args.env_name)
    ...
    if len(episode_returns) > 0:
        ...
        plt.title(f"{args.env_name} – PPO Training")
    ...
    csv_path = os.path.join("output", "PPO", "final_performance.csv")
