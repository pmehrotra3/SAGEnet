# A2C_agent.py
# A2C agent using Stable-Baselines3, talking to the Redis-based Gymnasium simulator.
...
class RedisEnv(gym.Env):
    ...
    Same protocol as in PPO_agent.py:
      - STATE_KEY: simulator pushes states
      - ACTION_KEY: agent pushes actions
      - EXPERIENCE_KEY: simulator pushes (next_state, reward, terminated)
    """
    ...
    def reset(self, *, seed=None, options=None):
        # Seeding of the remote env is controlled by the simulator, not here
        print("🔁 A2C Agent: Waiting for initial state from simulator...")
    ...
    def step(self, action):
        ...
        # Convert action to JSON-serializable form
        ...
        # Only consume state if not done
        if not done:
...
def main():
    parser = argparse.ArgumentParser(
        description="A2C agent that connects to the Redis Gymnasium simulator."
    )
    parser.add_argument(
        "env_name",
        type=str,
        help="Gymnasium environment name (e.g., 'CartPole-v1', 'LunarLander-v2').",
    )
    parser.add_argument(
        "--total_timesteps",
        type=int,
        default=100_000,
        help="Total training timesteps for A2C.",
    )
    parser.add_argument(
        "--redis-host",
        type=str,
    ...
    save_name = f"a2c_{args.env_name}"
    model.save(save_name)
    print(f"💾 A2C model saved as '{save_name}.zip'.")
