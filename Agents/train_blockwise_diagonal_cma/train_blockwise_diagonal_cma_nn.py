# train_blockwise_diagonal_cma_nn.py
# Trains BlockwiseDiagonalCMAnn (Custom implementation using pycma) on a Gymnasium environment and logs per-episode returns/lengths
# and per-episode system metrics (wall time, CPU time, RAM, CPU%) to CSV files.
#
# Usage: python train_blockwise_diagonal_cma_nn.py <env_name> [--total-timesteps N] [--num-runs N] [--block-size N]
#
...
from DiagonalBlockwiseCMAnn import DiagonalBlockwiseCMAnn
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train DiagonalBlockwiseCMAnn with full episode and system logging.")
    ...
    # --- output directory ---
    out_dir = os.path.join("..", "..", "output", args.env_name, "DiagonalBlockwiseCMAnn")
    ...
    for run in range(1, args.num_runs + 1):
        ...
        model  = DiagonalBlockwiseCMAnn(env=env, block_size=args.block_size)
