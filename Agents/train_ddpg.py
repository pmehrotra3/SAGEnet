# train_ddpg.py
# Trains DDPG (SB3) on a Gymnasium environment and logs per-episode returns/lengths
# and per-episode system metrics (wall time, CPU time, RAM, CPU%) to CSV files.
#
# Usage: python train_ddpg.py <env_name> [--total-timesteps N] [--render]
#
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train DDPG with full episode and system logging.")
    ...
    # --- output directory ---
    out_dir = os.path.join("..", "output", args.env_name, "DDPG")
    ...
    for run in range(1, args.num_runs + 1):
        ...
        model = DDPG(policy="MlpPolicy",  # standard feedforward network for vector observations
                    env=env,
