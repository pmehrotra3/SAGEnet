# train_a2c.py
# Trains A2C (SB3) on a Gymnasium environment and logs per-episode returns/lengths
# and per-episode system metrics (wall time, CPU time, RAM, CPU%) to CSV files.
#
# Usage: python train_a2c.py <env_name> [--total-timesteps N] [--render]
#
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train A2C with full episode and system logging.")
    ...
    # --- output directory ---
    out_dir = os.path.join("..", "output", args.env_name, "A2C")
    ...
    for run in range(1, args.num_runs + 1):
        ...
        model = A2C(policy="MlpPolicy",  # standard feedforward network for vector observations
                    env=env,
